  ...
  async function placeOrder() {
    ...
    try {
      ...
      const order = await createFakeOrder({
        items,
        address: address.trim() || "台北市信義區松仁路 100 號",
        paymentMethod:
    ...
    <>
      ...
      <main className="mx-auto max-w-2xl space-y-6 px-4 pb-32 pt-4">
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-zinc-500">{item.restaurantName}</p>
        ...
        <section className="space-y-2 rounded-2xl border border-border bg-surface p-4 text-sm">
          ...
          <div className="flex justify-between text-zinc-500">
            <span>外送費</span>
            <span>{formatNT(DELIVERY_FEE)}</span>
          </div>
          <div className="flex justify-between text-zinc-500">
            <span>平台服務費</span>
            <span>{formatNT(SERVICE_FEE)}</span>
          </div>
